...
function StatCard({ icon, label, value, iconBg }) {
  return (
    <div className="bg-[#0d1525] border border-[#1e3052] rounded-2xl p-5 flex items-center gap-5
      hover:border-indigo-500/30 transition-all ring-1 ring-white/3">
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm text-[#8899bb] mt-1">{label}</p>
      </div>
    </div>
  );
}
...
// ─── Status metadata ──────────────────────────────────────────────────────────
const STATUS_META = {
  todo:       { label: 'To Do',       bar: 'bg-slate-400',   badge: 'bg-slate-500/10 text-slate-400 ring-1 ring-slate-500/15' },
  inprogress: { label: 'In Progress', bar: 'bg-blue-500',    badge: 'bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/20' },
  done:       { label: 'Done',        bar: 'bg-emerald-500', badge: 'bg-emerald-500/15 text-emerald-400 ring-1 ring-emerald-500/20' },
};
...
function StatusBar({ status, count, total }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 bg-[#142038] rounded-full h-2 overflow-hidden">
        <div
      ...
      <div className="shrink-0 text-right w-20">
        ...
        <span className="text-xs text-[#8899bb] ml-1">({ratio}%)</span>
      </div>
    </div>
  );
}
...
function memberBadgeColor(ratio) {
  if (ratio >= 70) return 'text-emerald-400 bg-emerald-500/15 ring-1 ring-emerald-500/20';
  if (ratio >= 30) return 'text-amber-400 bg-amber-500/15 ring-1 ring-amber-500/20';
  return 'text-red-400 bg-red-500/15 ring-1 ring-red-500/20';
}

function MemberRow({ entry }) {
  const ratio = pct(entry.completedCount, entry.taskCount);
  return (
    <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-[#142038] transition-all">
      ...
      <div className="min-w-0 w-36 shrink-0">
        <p className="text-sm font-semibold text-white truncate">{entry.user.name}</p>
        <p className="text-xs text-[#8899bb] truncate">{entry.user.email}</p>
      </div>
      <div className="flex-1 min-w-0">
        <div className="w-full bg-[#142038] rounded-full h-2 overflow-hidden">
          <div
      ...
      <div className="shrink-0 flex items-center gap-2">
        <span className="text-xs text-[#8899bb]">{entry.completedCount}/{entry.taskCount}</span>
...
function OverdueRow({ task }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-red-500/[0.08] border border-red-500/20 rounded-xl">
      ...
      <span className={`shrink-0 text-xs font-medium px-2.5 py-1 rounded-full ${statusMeta?.badge ?? 'bg-slate-500/10 text-slate-400'}`}>
...
function Section({ title, icon, children, headerRight }) {
  return (
    <div className="bg-[#0d1525] border border-[#1e3052] rounded-2xl p-6 ring-1 ring-white/3">
...
export default function Dashboard() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-[#070b14] flex flex-col items-center justify-center gap-2 text-[#8899bb]">
        <FiAlertTriangle className="text-4xl text-red-400" />
        <p className="text-lg font-medium text-white">Could not load dashboard</p>
        <button onClick={() => navigate('/projects')} className="text-indigo-400 text-sm hover:text-indigo-300 transition-colors">
          Back to Projects
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#070b14]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          >
            {total === 0 ? (
              <div className="text-center py-8 text-[#48617a]">
                <FiList className="text-3xl mx-auto mb-2 opacity-40" />
                <p className="text-sm text-[#8899bb]">No tasks in this project yet</p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {total > 0 && (
                  <div className="mt-4 pt-4 border-t border-[#1a2844]">
                    <p className="text-xs text-[#48617a] mb-2 font-medium uppercase tracking-wide">Overall distribution</p>
          ...
          >
            {overdues.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-[#8899bb] mt-1">Everything is on schedule</p>
              </div>
            ) : (
        ...
        {isAdmin && (
          <Section
            ...
            headerRight={
              <span className="text-xs text-[#8899bb] font-medium">Completion rate per member</span>
            }
          >
            {(!tasksByUser || tasksByUser.length === 0) ? (
              <div className="text-center py-10 text-[#48617a]">
                <FiUsers className="text-3xl mx-auto mb-2 opacity-40" />
                <p className="text-sm text-[#8899bb]">No task assignments yet</p>
                <p className="text-xs text-[#48617a] mt-1">Assign tasks to team members to see stats here</p>
              </div>
            ) : (
              <>
                {/* Legend */}
                <div className="flex items-center gap-5 mb-4 pb-4 border-b border-[#1a2844] flex-wrap">
                  {[
                    ...
                    <div key={label} className="flex items-center gap-1.5 text-xs text-[#8899bb]">
                ...
                <div className="hidden sm:grid grid-cols-[1fr_2fr_1fr] gap-4 px-3 mb-1">
                  <span className="text-xs font-semibold text-[#48617a] uppercase tracking-wide">Member</span>
                  <span className="text-xs font-semibold text-[#48617a] uppercase tracking-wide">Progress</span>
                  <span className="text-xs font-semibold text-[#48617a] uppercase tracking-wide text-right">Tasks</span>
                </div>
                ...
                {project?.members && (() => {
                  ...
                  return (
                    <div className="mt-4 pt-4 border-t border-[#1a2844]">
                      <p className="text-xs font-medium text-[#48617a] mb-2">No assigned tasks</p>
                      <div className="flex flex-wrap gap-2">
                        {unassigned.map((m) => (
                          <div key={m.user._id} className="flex items-center gap-1.5 bg-[#142038] border border-[#1a2844] rounded-full px-3 py-1">
                            <div className="w-5 h-5 rounded-full bg-linear-to-br from-indigo-500/50 to-violet-600/50 text-[#8899bb] text-xs font-bold flex items-center justify-center">
                              {initials(m.user.name)}
                            </div>
                            <span className="text-xs text-[#8899bb]">{m.user.name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          >
            {total === 0 ? (
              <div className="text-center py-10 text-[#48617a]">
                ...
                <p className="text-sm text-[#8899bb]">No tasks assigned to you yet</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {['todo', 'inprogress', 'done'].map((s) => {
                  ...
                  return (
                    <div key={s} className="flex flex-col items-center justify-center p-6 rounded-2xl bg-[#070b14] border border-[#1a2844] gap-2 hover:border-[#253857] transition-all">
                      ...
                      <span className="text-xs text-[#48617a] mt-1">
